...
- **DuckLake**: A lakehouse format built on DuckDB with snapshot-based versioning, time-travel queries, and support for various metadata backends (DuckDB, PostgreSQL). Data is stored in Parquet format on local or object storage (see [DuckLake](8-ducklake.md)).
- **Iceberg**: Apache Iceberg catalogs with automatic table discovery, time-travel queries via snapshots, and standard DML operations. Supports REST, AWS Glue, and S3 Tables catalogs (see [Iceberg](9-iceberg.md)).
- **HTTP RESTful API**: Allows you to connect to any RESTful API endpoint (see [HTTP RESTful API](5-http.md)).
- **Hugr App**: Pluggable applications that connect via DuckDB Airport (Arrow Flight gRPC) and publish custom functions, tables, and data sources into the shared GraphQL schema. Apps self-register with hugr and support heartbeat monitoring, crash recovery, and versioned upgrades (see [Hugr App](10-hugr-app.md) and [Hugr Apps Developer Guide](/docs/hugr-apps)).
- **Airport**: Attach remote DuckDB catalogs via DuckDB Airport extension (Arrow Flight gRPC). Enables access to tables and functions served by any Airport-compatible endpoint (see [Airport](13-airport.md)).
- **LLM (AI Models)**: Connect to AI model providers (OpenAI, Anthropic, Gemini) for text generation and tool calling. Supports per-source rate limiting (rpm/tpm) with Redis shared counters or in-memory fallback. Accessed via the `core.models` runtime module (see [LLM](11-llm.md) and [AI Models Guide](/docs/ai-models)).
- **Redis**: Key-value store for caching, counters, rate limiting, and shared state. Accessed via the `core.store` runtime module (see [Redis](12-redis.md)).
...
`hugr` uses relational database data sources (DuckDB, PostgreSQL, MySQL, SQL Server, DuckLake, Iceberg) as attached databases to the DuckDB query engine. You can explore the internal data source structure (tables, views, constraints) by using metadata queries via views in the `core.meta` module.
