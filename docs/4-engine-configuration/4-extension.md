...
extend type pg_users @dependency(name: "analytics") {
  # Can now reference types from "analytics" data source
}
```
...
**Explanation**:
- `pg_users` - type from PostgreSQL (prefix `pg`)
- `mysql_user_sessions` - type from MySQL (prefix `mysql`)
- `@dependency(name: "analytics")` - declares dependency on the MySQL analytics data source
...
```graphql
extend type pg_orders @dependency(name: "warehouse") @dependency(name: "crm") {
...
extend type pg_orders @dependency(name: "shipping_api") {
...
```graphql
extend type pg_products @dependency(name: "recommendation_engine") {
  ...
  @view(
    ...
    sql: """
      SELECT
        ...
        c.lifetime_value
      FROM pg_orders o
      INNER JOIN mysql_customers c ON o.customer_id = c.customer_id
...
- Can join tables from different sources: `pg_orders` and `mysql_customers`
...
type regional_sales_summary
  @view(
    ...
    sql: """
      ...
      FROM pg_regions r
      LEFT JOIN pg_orders o ON o.region_id = r.region_id
...
extend type pg_customers
  @dependency(name: "analytics")
  @dependency(name: "warehouse")
  @dependency(name: "shipping_api")
{
...
type customer_order_analytics
  @view(
    ...
    sql: """
      ...
      FROM pg_customers c
      LEFT JOIN pg_orders o ON c.id = o.customer_id
      LEFT JOIN mysql_user_behavior b ON c.email = b.user_email
  ...
  @view(
    sql: """
      SELECT
        ...
        o.id, o.total, c.name
      FROM pg_orders o
      INNER JOIN mysql_customers c ON o.customer_id = c.id
...
```graphql
extend type pg_products @dependency(name: "analytics") {
...
type unified_customer_360
  @view(sql: """
    ...
    FROM pg_customers c
    LEFT JOIN pg_orders o ON c.id = o.customer_id
    LEFT JOIN mysql_sessions s ON c.email = s.user_email
    LEFT JOIN crm_interactions i ON c.id = i.customer_id
