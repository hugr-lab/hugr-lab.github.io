...
- **SQL Server / Azure SQL**: Microsoft SQL Server and Azure Fabric Warehouse / Analytical Endpoints. Supports SQL Server authentication (`mssql://` protocol) and Azure EntraID service principal authentication (`azure://` protocol).
- **DuckLake**: A data lake solution that supports various storage systems, including cloud storage and distributed file systems. DuckLake is designed to handle large volumes of data, provides efficient querying capabilities, and is able to manage data and schema changes through snapshots. The catalog can live in PostgreSQL, DuckDB, or [SQL Server / Azure SQL](https://hugr-lab.github.io/mssql-ducklake/) through the mssql_ducklake extension.
- **Apache Iceberg**: An open table format for large-scale analytic datasets. Hugr supports Iceberg catalogs as data sources through DuckDB's iceberg extension, providing automatic table discovery, time-travel queries via snapshots, and standard DML operations. Works with any Iceberg REST catalog (Apache Polaris, Lakekeeper), AWS Glue, and S3 Tables.
...
The ingest pipeline is rule-based, processing schema definitions through a sequence of validation and preparation rules. What it stores is the **logical model** of the source — its modules, data objects, fields, relations and functions — not a finished GraphQL schema; the served surface (filters, aggregations, mutation inputs, navigation fields, module roots) is generated from that model **on read**. Reloading a source whose definition has not changed is therefore free: the stored model is reused as is. Extension sources track dependencies on their base sources and are automatically suspended and reactivated when a base source reloads.

At data source loading time, `hugr` combines all catalog sources into a single source, validates it, and writes the resulting logical model to the core database.
