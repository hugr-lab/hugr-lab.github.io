...
The schema compiler is rule-based, processing schema definitions through a pipeline of validation and generation rules. This architecture enables incremental compilation — when a data source schema changes, only the affected parts are recompiled rather than rebuilding the entire schema. Extension sources track dependencies on their base sources and are automatically suspended/reactivated when a base source reloads.
